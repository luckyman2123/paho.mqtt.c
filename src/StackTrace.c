...
typedef struct
{
	thread_id_type threadid;					// comment by Clark:: 线程id    ::2021-3-25
	char name[MAX_FUNCTION_NAME_LENGTH];		// comment by Clark:: 函数名  ::2021-3-25
	int line;									// comment by Clark:: 行号      ::2021-3-25
} stackEntry;

typedef struct
{
	thread_id_type id;			// comment by Clark:: 线程id号               ::2021-3-25
	int maxdepth;				// comment by Clark:: 达到过最大的深度             ::2021-3-25
...
static threadEntry threads[MAX_THREADS];		// comment by Clark:: 最多支持查寻255个线程    ::2021-3-25
...
void StackTrace_entry(const char* name, int line, enum LOG_LEVELS trace_level)
{
	...
	if (!setStack(1))			// comment by Clark:: 会使用到 my_thread 全局变量         ::2021-3-25
		goto exit;
...
// comment by Clark:: 退出函数, 需要匹配名称, 如果名称不匹配, 则报错  ::2021-3-25
...
void StackTrace_printStack(FILE* dest)
{
	...
	for (t = 0; t < thread_count; ++t)
	{
		...
		// comment by Clark:: id 大于0, 代表使用过  ::2021-3-25
		if (cur_thread->id > 0)
		{
